...
async function deduplicateEmployees() {
  ...
  // 1. Delete ALL current employees to start fresh
  const { error: delError } = await supabase.from('employees').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  ...
  // 2. Insert the 3 unique employees from your screenshot
  const employees = [
    { 
      name: 'Grade Test', 
      emp_code: 'GT-101', 
