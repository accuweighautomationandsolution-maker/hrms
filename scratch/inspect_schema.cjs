...
async function inspectSchema() {
  console.log('Attempting dummy insert to find column names...');
  const { data, error } = await supabase
    .from('employees')
    .insert({ name: 'Schema Test' })
    .select();
  
  if (error) {
     console.error('Insert failed:', error.message);
  } else {
     console.log('Insert success! Columns are:', Object.keys(data[0]));
     // Cleanup
     await supabase.from('employees').delete().eq('name', 'Schema Test');
  }
}
